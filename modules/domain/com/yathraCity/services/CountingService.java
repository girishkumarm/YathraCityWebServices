...
public class CountingService{
	...
	@Path("/count-number-users")
	public com.yathraCity.core.Message countNumberOfUsers(){
		com.yathraCity.core.Message resp=null;
		SessionUtils.clear(request);
		ServiceExecutionContext context=null;
		try
		{
			context = new ServiceExecutionContext(request);
			CountingServiceInterface intfc =  (CountingServiceInterface) (ServiceParser.getImpl("com.yathraCity.services.CountingService"));
			if (intfc == null) throw new ExecException(ErrorCodes.APPLICATION_ERROR,null,"Service not implemented");
			resp =  intfc.countNumberOfUsers(context);
			SessionUtils.clear(request);
			return resp;
